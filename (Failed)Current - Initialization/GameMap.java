...
//Log 02/11/2017: Today is the last day of the contest, and I only have one more thing to do: Map Analyze
//Thought: Send Mesages for the above half highest production
public class GameMap{
    ...
    public double highestValue, averageValue, lowestValue;
    ...
    void mapAnalyze() {
        determineMyHighest();
        int topRegionBottomLimit = highest / 2;
        List<Mesage> highProduction = new ArrayList<Mesage>(100);
        for (int y = 0; y < height; y++) 
            for (int x = 0; x < width; x++) 
                locations[x][y].site.initialize();
        for (int y = 0; y < height; y++) 
            for (int x = 0; x < width; x++)
                if (locations[x][y].site.production >= topRegionBottomLimit){// | (locations[x][y].site.production > 0 & locations[x][y].site.strength <= 25)){
                    ...
                    highProduction.add(new Mesage(loc1.x, loc1.y, oppositeDirection(Direction.EAST), locations[x][y].site.initValue));
                    highProduction.add(new Mesage(loc2.x, loc2.y, oppositeDirection(Direction.SOUTH), locations[x][y].site.initValue));
                    highProduction.add(new Mesage(loc3.x, loc3.y, oppositeDirection(Direction.WEST), locations[x][y].site.initValue));
                    highProduction.add(new Mesage(loc4.x, loc4.y, oppositeDirection(Direction.NORTH), locations[x][y].site.initValue));
                    locations[x][y].site.analyzed = true;
                }
        setValues(highProduction);
    }
    void setValues(List<Mesage> collection) {
        List<Mesage> secondCollection = new ArrayList<Mesage>(100);
        for (Mesage temp: collection) locations[temp.x][temp.y].site.editValue(temp.value, temp.direction);
        for (Mesage temp: collection) locations[temp.x][temp.y].site.getMapValue();
        for (Mesage temp: collection) {
            ...
            if (loc1.site.getMapValue() > locations[temp.x][temp.y].site.getMapValue() & temp.direction != Direction.WEST) secondCollection.add(new Mesage(loc1.x, loc1.y, Direction.EAST, loc1.site.mapValue));
            if (loc2.site.getMapValue() > locations[temp.x][temp.y].site.getMapValue() & temp.direction != Direction.NORTH) secondCollection.add(new Mesage(loc2.x, loc2.y, Direction.SOUTH, loc2.site.mapValue));
            if (loc3.site.getMapValue() > locations[temp.x][temp.y].site.getMapValue() & temp.direction != Direction.EAST) secondCollection.add(new Mesage(loc3.x, loc3.y, Direction.WEST, loc3.site.mapValue));
            if (loc4.site.getMapValue() > locations[temp.x][temp.y].site.getMapValue() & temp.direction != Direction.SOUTH) secondCollection.add(new Mesage(loc4.x, loc4.y, Direction.NORTH, loc4.site.mapValue));
        }
        if (secondCollection.size() !=0) setValues(secondCollection);
    }
    ...
    void determineMyHighest() {//Basically Preparing for the real game: going through the whole map without give 
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                {
                    myTotal++;
                    averageValue += locations[x][y].site.value;
                    ...
                    if (s1.owner == 0 & s1.getMapValue() < lowestValue) lowestValue = 1.0*s1.mapValue;
                    if (s2.owner == 0 & s2.getMapValue() < lowestValue) lowestValue = 1.0*s2.mapValue;
                    if (s3.owner == 0 & s3.getMapValue() < lowestValue) lowestValue = 1.0*s3.mapValue;
                    if (s4.owner == 0 & s4.getMapValue() < lowestValue) lowestValue = 1.0*s4.mapValue;
        ...
        averageValue = averageValue / myTotal;
    }
                    ...
                    else if ((s1.owner == 0 | s2.owner == 0 | s3.owner == 0 | s4.owner == 0) & ((myTotal < total*3/4)))//& (!enemyNear)))
                    {
                        ...
                        if (s1.owner == 0 & ((attackStarted | !s1.dangerous | myTotal > total*2/5) & (!(s1.production < 1 & frame < 150) | (locations[x][y].site.production > 3 & s1.strength <= 30))))// & s1.production >= myHighest)//No enemy around, but with a non-Agressive neighbor
                        {
                            if (myTotal > firstTargetLength){
                                ...
                                else if (myTotal > firstTargetLength)
                                {
                            ...
                            if (1.0 * s1.mapValue <= lowestValue + 0.1) {
                        ...
                        if (!pass & (s2.owner == 0 & ((attackStarted | !s2.dangerous | myTotal > total*2/5) & (!(s2.production < 1 & frame < 150) | (locations[x][y].site.production > 3 & s2.strength <= 30)))))// & s2.production >= myHighest)
                        {
                            if (myTotal > firstTargetLength){
                                if (/*highestcount < 10 & */s2.production >= myHighest - modify) 
                                {
                                    ...
                                    if (1.0*s2.production/s2.strength > value & myTotal > firstTargetLength) {
                            ...
                            if (1.0 * s2.mapValue <= lowestValue + 0.1) {
                        ...
                        if (!pass & (s3.owner == 0 & ((attackStarted | !s3.dangerous | myTotal > total*2/5) & (!(s3.production < 1 & frame < 150) | (locations[x][y].site.production > 3 & s3.strength <= 30)))))// & s3.production >= myHighest)
                        {
                            if (myTotal > firstTargetLength){
                                if (/*highestcount < 10 & */s3.production >= myHighest - modify) 
                                {
                                    ...
                                    if (1.0*s3.production/s3.strength > value & myTotal > firstTargetLength) {
                            ...
                            if (1.0 * s3.mapValue <= lowestValue + 0.1) {
                        ...
                        if (!pass & (s4.owner == 0 & ((attackStarted | !s4.dangerous | myTotal > total*2/5) & (!(s4.production < 1 & frame < 150) | (locations[x][y].site.production > 3 & s4.strength <= 30)))))//& s4.production >= myHighest)
                        {
                            if (myTotal > firstTargetLength)
                            {
                            ...
                            if (1.0 * s4.mapValue <= lowestValue + 0.1) {
    ...
    void setDirection(List<Pair> collection, int turn) {
        ...
        for (Pair temp : collection){
            if (((temp.mod > 4) & turn <= 12)|(temp.mod == 4 & turn < 20)|(temp.mod < 4 & turn <= length)) {//Bug Fixed!
            Location loc1 = getLocation(locations[temp.x][temp.y], Direction.WEST);
            Location loc2 = getLocation(locations[temp.x][temp.y], Direction.NORTH);
            Location loc3 = getLocation(locations[temp.x][temp.y], Direction.EAST);
            Location loc4 = getLocation(locations[temp.x][temp.y], Direction.SOUTH);
    ...
    void finalCheck() {
                ...
                if (locations[x][y].site.owner == myID & !locations[x][y].site.decided) {
                    ...
                    if ((siteFinal.owner == 0 | siteFinal.owner == myID) & siteFinal.strength < 40) {//Target is an enemy
                        ...
                        if (allyDirection.size() != 0) {
                            ...
                            if (locations[x][y].site.strength + ally.strength > 256) {
                            ...
                            else if (locations[x][y].site.strength + ally.strength < 256){
        ...
        for (int y = 0; y < height; y++) //Let the Big ones in the middle of the territory swap 
            for (int x = 0; x < width; x++) 
                ...
                else if (siteFinal.owner == myID & futureStrength(x, y, Direction.STILL) > 256) sendWait(x, y, oppositeDirection(locations[x][y].site.finalMove));
                    }
                }
    }
    ...
    void sendWaitExcept(int x, int y, Direction fromDirection, int swapperStrength) {
        ...
        if (allyCollection.size() > 0) {
            ...
            Site ally = locAlly.getSite();
            locations[x][y].site.decided = true;
            if (ally.strength + swapperStrength > 256) sendWait(locAlly.x, locAlly.y, oppositeDirection(tempDirection));
    ...
    Direction preMove(int x, int y) {
        ...
        if (tempSite.owner == myID) {
            if (locations[x][y].site.strength >= minAttackMod*locations[x][y].site.production - 1) dr = locations[x][y].site.preferedMove;
