...
//Need to Improve: stop the big guys from overemerging (WASTE!!!) -- Run through every tile, given postFinalMove as well as futureStrength -- then stop some guys or swith them;
public class GameMap{
    ...
    Direction checkInDanger(int x, int y) {//In Test Mode
        ...
        if (s1.owner == 0) st1 = 1 + enemyStrengthTotal(loc1.x, loc1.y);// + leftRight[y]/2;
        if (s2.owner == 0) st2 = 1 + enemyStrengthTotal(loc2.x, loc2.y);// + upDown[x]/2;
        if (s3.owner == 0) st3 = 1 + enemyStrengthTotal(loc3.x, loc3.y);// + leftRight[y]/2;
        if (s4.owner == 0) st4 = 1 + enemyStrengthTotal(loc4.x, loc4.y);// + upDown[x]/2;
            ...
            for (int x = 0; x < width; x++) {
                if (locations[x][y].site.owner == myID)
                {
                    ...
                    else if ((s1.owner == 0 | s2.owner == 0 | s3.owner == 0 | s4.owner == 0) & ((myTotal < total*2/3)))//& (!enemyNear)))
                    {
                        ...
                        boolean exist = false;
                        Pair p = new Pair(x, y, Direction.STILL, 8);
                        double value = 0;
                        if (s1.owner == 0 & ((attackStarted | !s1.dangerous | myTotal > total/2) & (!(s1.production < 1 & frame < 150) | s1.strength <= 30)))// & s1.production >= myHighest)//No enemy around, but with a non-Agressive neighbor
                        {
                            if (/*highestcount < 10 & */s1.production >= myHighest - 1) 
                            {
                        ...
                        if (s2.owner == 0 & ((attackStarted | !s2.dangerous | myTotal > total/2) & (!(s2.production < 1 & frame < 150) | s2.strength <= 30)))// & s2.production >= myHighest)
                        {
                            if (/*highestcount < 10 & */s2.production >= myHighest - 1) 
                            {
                        ...
                        if (s3.owner == 0 & ((attackStarted | !s3.dangerous | myTotal > total/2) & (!(s3.production < 1 & frame < 150) | s3.strength <= 30)))// & s3.production >= myHighest)
                        {
                            if (/*highestcount < 10 & */s3.production >= myHighest - 1) 
                            {
                        ...
                        if (s4.owner == 0 & ((attackStarted | !s4.dangerous | myTotal > total/2) & (!(s4.production < 1 & frame < 150) | s4.strength <= 30)))//& s4.production >= myHighest)
                        {
                            if (/*highestcount < 10 & */s4.production >= myHighest - 1) 
                            {
    ...
    void setDirection(List<Pair> collection, int turn) {
        ...
        for (Pair temp : collection){
            if ((((temp.mod == 8)|(temp.mod == 1)) & turn <= 6)|(turn < length)) {
    ...
    Direction finalMove(int x, int y) {
        ...
        if (tempSite.owner == myID) {
            ...
            if (locations[x][y].site.strength > 12) dr = locations[x][y].site.preferedMove;
            else dr = Direction.STILL;
        }
        else {
