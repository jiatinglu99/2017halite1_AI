...
public class GameMap{
    ...
    Direction anyOneIsMovingIn(int x, int y) {// In this place, I should detect if there are more than one allies coming in
        final Site s1 = getSite(locations[x][y], Direction.WEST);
        final Site s2 = getSite(locations[x][y], Direction.NORTH);
        final Site s3 = getSite(locations[x][y], Direction.EAST);
        final Site s4 = getSite(locations[x][y], Direction.SOUTH);
        Direction allyDirection = Direction.STILL; //This part is not done yet(More Features to add)
        if (s1.owner == myID & s1.finalMove == Direction.EAST)//If an Ally moving in
            allyDirection = Direction.WEST;
        else if (s2.owner == myID & s2.finalMove == Direction.SOUTH)
            allyDirection = Direction.NORTH;
        else if (s3.owner == myID & s3.finalMove == Direction.WEST)
            allyDirection = Direction.EAST;
        else if (s4.owner == myID & s4.finalMove == Direction.NORTH)
            allyDirection = Direction.SOUTH;
