...
public class GameMap{
            ...
            for (int x = 0; x < width; x++) {
                if (locations[x][y].site.owner == myID)
                {
                    ...
                    if (s1.owner != myID & s1.owner != 0 & s1.strength > 0)//At least one is actual enemy
                    {
                        ...
                        Pair p = new Pair(x, y, Direction.WEST, 0, 0);
                        collection.add(p);
                        //enemyNear = true;
                    }
                    ...
                    {
                        //locations[x][y].site.setDangerous();
                        Pair p = new Pair(x, y, Direction.NORTH, 0, 0);
                        collection.add(p);
                        //enemyNear = true;
                    }
                    ...
                    {
                        //locations[x][y].site.setDangerous();
                        Pair p = new Pair(x, y, Direction.EAST, 0, 0);
                        collection.add(p);
                        //enemyNear = true;
                    }
                    ...
                    {
                        //locations[x][y].site.setDangerous();
                        Pair p = new Pair(x, y, Direction.SOUTH, 0, 0);
                        collection.add(p);
                        //enemyNear = true;
                    }
                    ...
                    {
                        //locations[x][y].site.setDangerous();
                        Pair p = new Pair(x, y, checkInDanger(x, y), 0, 0);
                        collection.add(p);
                        //enemyNear = true;
                    }
                    ...
                    else if ((s1.owner == 0 | s2.owner == 0 | s3.owner == 0 | s4.owner == 0) & ((myTotal < total/2)))//& (!enemyNear)))
                    {
                        //locations[x][y].site.setBrim();
                        if (s1.owner == 0 & (s1.production >= myHighest | s1.production >= 3 | locations[x][y].site.strength - 5 > s1.strength))//No enemy around, but with a non-Agressive neighbor
                        {
                            Pair p;
                            if (s1.production >= highest) p= new Pair(x, y, Direction.WEST, 0, 0);
                            else p= new Pair(x, y, Direction.WEST, 4, s1.strength - locations[x][y].site.strength);
                            collection.add(p);
                        }    //locations[x][y].site.editDirection(Direction.WEST, highest-s1.production);
                        if (s2.owner == 0 & (s2.production >= myHighest | s2.production >= 3 | locations[x][y].site.strength - 5 > s2.strength))
                        {
                            Pair p;
                            if (s2.production >= highest) p= new Pair(x, y, Direction.WEST, 0, 0);
                            else p= new Pair(x, y, Direction.WEST, 4, s2.strength - locations[x][y].site.strength);
                            collection.add(p);
                        }
                            //locations[x][y].site.editDirection(Direction.NORTH, highest-s2.production);
                        if (s3.owner == 0 & (s3.production >= myHighest | s3.production >= 3 | locations[x][y].site.strength - 5 > s3.strength))
                        {
                            Pair p;
                            if (s3.production >= highest) p= new Pair(x, y, Direction.WEST, 0, 0);
                            else p= new Pair(x, y, Direction.WEST, 4, s2.strength - locations[x][y].site.strength);
                            collection.add(p);
                        }
                            //locations[x][y].site.editDirection(Direction.EAST, highest-s3.production);
                        if (s4.owner == 0 & (s4.production >= myHighest | s4.production >= 3 | locations[x][y].site.strength - 5 > s4.strength))
                        {
                            Pair p;
                            if (s4.production >= highest) p= new Pair(x, y, Direction.WEST, 0, 0);
                            else p= new Pair(x, y, Direction.WEST, 4, s2.strength - locations[x][y].site.strength);
                            collection.add(p);
                        }
    ...
    void setDirection(List<Pair> collection, int turn) {
        ...
        for (Pair temp : collection){
            if ((temp.mod == 4 & temp.need > 0)|(temp.mod == 0 & turn < length)) {
            ...
            {
                Pair f = new Pair(loc1.x, loc1.y, Direction.EAST, temp.mod, temp.need - s1.strength);
                secondCollection.add(f);
            }
            if (s2.owner == myID & s2.min > turn + 1) 
            {
                Pair f = new Pair(loc2.x, loc2.y, Direction.SOUTH, temp.mod, temp.need - s2.strength);
                secondCollection.add(f);
            }
            if (s3.owner == myID & s3.min > turn + 1) 
            {
                Pair f = new Pair(loc3.x, loc3.y, Direction.WEST, temp.mod, temp.need - s3.strength);
                secondCollection.add(f);
            }
            if (s4.owner == myID & s4.min > turn + 1) 
            {
                Pair f = new Pair(loc4.x, loc4.y, Direction.NORTH, temp.mod, temp.need - s4.strength);
                secondCollection.add(f);
            }
        }}}
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                if (locations[x][y].site.owner == myID)
                {
                    ...
                    {
                        if (locations[x][y].site.strength > 30) dr = locations[x][y].site.preferedMove;
                    ...
                    {
                        if (locations[x][y].site.strength >= tempSite.strength + 1) dr = locations[x][y].site.preferedMove;
                        else dr = Direction.STILL;
                    }
