...
public class GameMap{
    ...
    void determineMyHighest() {//Determine the current highest(-1) produciton; Also, determine if the untaken spot is in danger(If I just take it, it will result war which I might not be prepared)
        myHighest = 1;
        myTotal = 0;
        highestCount = 0;
        enemyNear = false;
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                if (locations[x][y].site.owner == myID)
                {
                    ...
                    if (s1.owner == 0 & s1.production > myHighest & s1.strength != 0) myHighest = s1.production - 1; //temporary change
                    if (s2.owner == 0 & s2.production > myHighest & s2.strength != 0) myHighest = s2.production - 1; 
                    if (s3.owner == 0 & s3.production > myHighest & s3.strength != 0) myHighest = s3.production - 1; 
                    if (s4.owner == 0 & s4.production > myHighest & s4.strength != 0) myHighest = s4.production - 1; 
                ...
                else if (locations[x][y].site.owner == 0)
                {
                    final Site s1 = getSite(locations[x][y], Direction.WEST);
                    final Site s2 = getSite(locations[x][y], Direction.NORTH);
                    final Site s3 = getSite(locations[x][y], Direction.EAST);
                    final Site s4 = getSite(locations[x][y], Direction.SOUTH);
                    if ((s1.owner != 0 & s1.owner !=myID) | (s2.owner != 0 & s2.owner !=myID) | (s3.owner != 0 & s3.owner !=myID) | (s4.owner != 0 & s4.owner !=myID)) locations[x][y].site.dangerous = false;
                }
            }
        }
    }
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                if (locations[x][y].site.owner == myID)
                {
                    ...
                    else if ((s1.owner == 0 | s2.owner == 0 | s3.owner == 0 | s4.owner == 0) & ((myTotal < total)))//& (!enemyNear)))
                    {
                        //locations[x][y].site.setBrim();
                        if (s1.owner == 0 & s1.production >= myHighest & !s1.dangerous)//No enemy around, but with a non-Agressive neighbor
                        {
                            Pair p = new Pair(x, y, Direction.WEST, 5);
                            collection.add(p);
                        }    //locations[x][y].site.editDirection(Direction.WEST, highest-s1.production);
                        if (s2.owner == 0 & s2.production >= myHighest & !s2.dangerous)
                        {
                            Pair p = new Pair(x, y, Direction.NORTH, 5);
                            collection.add(p);
                        }
                            //locations[x][y].site.editDirection(Direction.NORTH, highest-s2.production);
                        if (s3.owner == 0 & s3.production >= myHighest & !s3.dangerous)
                        {
                            Pair p = new Pair(x, y, Direction.EAST, 5);
                            collection.add(p);
                        }
                            //locations[x][y].site.editDirection(Direction.EAST, highest-s3.production);
                        if (s4.owner == 0 & s4.production >= myHighest & !s4.dangerous)
                        {
                            Pair p = new Pair(x, y, Direction.SOUTH, 5);
                            collection.add(p);
                        }
    ...
    void setDirection(List<Pair> collection, int turn) {
        ...
        for (Pair temp : collection){
            if ((temp.mod == 4 & turn < length - 4)|(temp.mod == 0 & turn < length)) {
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                if (locations[x][y].site.owner == myID)
                {
                    ...
                    {
                        if (locations[x][y].site.strength > 40) dr = locations[x][y].site.preferedMove;
                    ...
                    if (locations[x][y].site.strength > 200 & dr == Direction.STILL) dr = Direction.NORTH;
